	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="测试类别" prop="ceshileibie">
					<el-select :disabled="ro.ceshileibie" v-model="ruleForm.ceshileibie" placeholder="请选择测试类别" >
						<el-option
							v-for="(item,index) in ceshileibieOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="测试类别" prop="ceshileibie">
					<el-input v-model="ruleForm.ceshileibie"
						placeholder="测试类别" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.cepingwenjian" label="测评文件" prop="cepingwenjian">
					<file-upload
						tip="点击上传测评文件"
						action="file/upload"
						:limit="1"
						:type="3"
						:multiple="true"
						:fileUrls="ruleForm.cepingwenjian?ruleForm.cepingwenjian:''"
						@change="cepingwenjianUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.cepingwenjian" label="测评文件" prop="cepingwenjian">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"rgba(44, 55, 66, 0.8)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.cepingwenjian)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.cepingwenjian" label="测评文件" prop="cepingwenjian">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="测试详情" prop="ceshixiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.ceshixiangqing" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.ceshixiangqing" label="测试详情" prop="ceshixiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.ceshixiangqing"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				ceshileibie: [
					{ required: true, message: '测试类别不能为空', trigger: 'blur' },
				],
				cepingwenjian: [
					{ required: true, message: '测评文件不能为空', trigger: 'blur' },
				],
				fabushijian: [
				],
				ceshixiangqing: [
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.ceshixiangqing = this.ruleForm.ceshixiangqing.replace(reg,'../../../pythons93f13ht/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.cepingwenjian!=null) {
		this.ruleForm.cepingwenjian = this.ruleForm.cepingwenjian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `xinlipingce/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xinlipingceCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    cepingwenjianUploadChange(fileUrls) {
	    this.ruleForm.cepingwenjian = fileUrls;
    },
  }
};
</script>
