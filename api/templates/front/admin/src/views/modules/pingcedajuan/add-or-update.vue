	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.datiwenjian" label="答题文件" prop="datiwenjian">
					<file-upload
						tip="点击上传答题文件"
						action="file/upload"
						:limit="1"
						:type="3"
						:multiple="true"
						:fileUrls="ruleForm.datiwenjian?ruleForm.datiwenjian:''"
						@change="datiwenjianUploadChange"
					></file-upload>
				</el-form-item>  
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.datiwenjian" label="答题文件" prop="datiwenjian">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"rgba(44, 55, 66, 0.8)","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.datiwenjian)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="!ruleForm.datiwenjian" label="答题文件" prop="datiwenjian">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				...
				datiwenjian: [
					{ required: true, message: '答题文件不能为空', trigger: 'blur' },
				],
				fengmian: [
				],
				xuehao: [
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='datiwenjian'){
							this.ruleForm.datiwenjian = obj[o];
							this.ro.datiwenjian = true;
							continue;
						}
						if(o=='fengmian'){
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.huifuneirong = this.ruleForm.huifuneirong.replace(reg,'../../../pythons93f13ht/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.datiwenjian!=null) {
		this.ruleForm.datiwenjian = this.ruleForm.datiwenjian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `pingcedajuan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.pingcedajuanCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    datiwenjianUploadChange(fileUrls) {
	    this.ruleForm.datiwenjian = fileUrls;
    },
