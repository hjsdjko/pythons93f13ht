...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
	<view class="content">
		<view :style='{"minHeight":"100vh","padding":"24rpx 0 24rpx","alignItems":"flex-start","flexWrap":"wrap","background":"#f2f2f2","display":"block","width":"100%","position":"relative","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"border":"0px solid #ff9593","padding":"0","margin":"0 auto","borderRadius":"12rpx","background":"none","display":"flex","width":"calc(100% - 48rpx)","height":"auto"}'>
				...
				<view :style='{"margin":"0 0px","borderColor":"#ff9593","borderRadius":"0","flex":"1","background":"#fff","borderWidth":"0","lineHeight":"80rpx","position":"relative","borderStyle":"solid","height":"80rpx"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.ceshimingcheng" type="text" placeholder="测试名称" ></input>
				</view>
				<view :style='{"margin":"0 0px","borderColor":"#ff9593","borderRadius":"0","flex":"1","background":"#fff","borderWidth":"0","lineHeight":"80rpx","position":"relative","borderStyle":"solid","height":"80rpx"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont " :style='{"color":"#999","left":"0px","textAlign":"center","width":"80rpx","fontSize":"40rpx","lineHeight":"80rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#666","borderRadius":"0","background":"none","width":"100%","lineHeight":"80rpx","fontSize":"28rpx","height":"80rpx"}' v-model="searchForm.ceshileibie" type="text" placeholder="测试类别" ></input>
				</view>
			...
			<view :style='{"padding":"0","alignItems":"flex-start","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
				<scroll-view scroll-y="true" class="category-two" :style='{"minHeight":"100vh","margin":"40rpx 0px 0 0","borderRadius":"0px 0 0 0px","background":"#f2f2f2","display":"block","width":"200rpx","height":"100%"}'>
					<view :class='categoryName === item.ceshileibie ? "active" : ""' class="tab" v-for="(item,index) in categoryList" :key="index" @tap="categoryClick(item.ceshileibie)">{{item.ceshileibie}}</view>
				</scroll-view>
			<!-- 样式2 -->
			<view class="list" :style='{"minHeight":"100vh","padding":"32rpx 24rpx 24rpx","margin":"40rpx auto 0","borderRadius":"0px","background":"#ffffff","flex":"1","width":"calc(100% - 200rpx)","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"border":"0px solid #eee","padding":"0px","margin":"0 0 40rpx","borderRadius":"12rpx","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"0px","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-if="preHttp(product.fengmian)" :src="product.fengmian.split(',')[0]"></image>
					<image :style='{"width":"200rpx","objectFit":"cover","borderRadius":"0px","display":"block","height":"200rpx"}' mode="aspectFill" class="listmpic" v-else :src="product.fengmian?baseUrl+product.fengmian.split(',')[0]:''"></image>
					...
					<view class="listmr" :style='{"padding":"0 0 0 10rpx","margin":"0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"calc(100% - 200rpx)","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.ceshimingcheng}}</view>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.ceshileibie}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinlipingce','修改')) || (!userid && isAuthFront('xinlipingce','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinlipingce','删除')) || (!userid && isAuthFront('xinlipingce','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinlipingce','修改')) || (!userid && isAuthFront('xinlipingce','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('xinlipingce','删除')) || (!userid && isAuthFront('xinlipingce','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
		...
		<button :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#38bf3e","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="userid && isAuth('xinlipingce','新增')" class="add-btn" @click="onAddTap()">新增</button>
		<button :style='{"border":"0","boxShadow":"none","color":"#fff","bottom":"128rpx","right":"128rpx","borderRadius":"100%","background":"#38bf3e","width":"80rpx","lineHeight":"80rpx","fontSize":"28rpx","position":"absolute","height":"80rpx","zIndex":"999"}' v-if="!userid && isAuthFront('xinlipingce','新增')" class="add-btn" @click="onAddTap()">新增</button>
	</view>
</mescroll-uni>
</template>
...
<script>
	export default {
		...
		async onShow() {
            ...
            let res = {};
            if(this.userid) {
                res = await this.$api.page('ceshileibie', {page:1,limit:100});
            } else {
                res = await this.$api.list('ceshileibie', {page:1,limit:100});
            }
			res.data.list.splice(0,0,{id:0,ceshileibie:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.ceshileibie = '%' + this.categoryName + '%'
				}
				if(this.searchForm.ceshimingcheng){
					params['ceshimingcheng'] = '%' + this.searchForm.ceshimingcheng + '%'
				}
				if(this.searchForm.ceshileibie){
					params['ceshileibie'] = '%' + this.searchForm.ceshileibie + '%'
				}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`xinlipingce`, params);
                } else {
                    res = await this.$api.list(`xinlipingce`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xinlipingce', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.search()
						}
			...
			async search(){
				...
				if(this.searchForm.ceshileibie){
					searchForm['ceshileibie'] = '%' + this.searchForm.ceshileibie + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`xinlipingce`, searchForm);
                } else {
                    res = await this.$api.list(`xinlipingce`, searchForm);
                }
				// 如果是第一页数据置空
